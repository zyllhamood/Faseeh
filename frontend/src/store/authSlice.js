...
export const login = createAsyncThunk('auth/login', async (data, thunkAPI) => {
    ...
    try {
        const response = await axios.post('http://172.20.10.5:8000/login/', data, {
            headers: {
...
export const register = createAsyncThunk('auth/register', async (data, thunkAPI) => {
    const { rejectWithValue } = thunkAPI;
    try {
        const response = await axios.post('http://172.20.10.5:8000/register/', data, {
            headers: {
                'Content-Type': 'application/json',
...
export const get_info = createAsyncThunk('auth/info', async (data, thunkAPI) => {
    ...
    try {
        ...
        const response = await axios.get('http://172.20.10.5:8000/profile/', {
            headers: {
                'Content-Type': 'application/json',
